classdef FuelCell_c < handle %goofy matlab class inheritance
    ...
    methods
        ...
        function plot_VoltagevsPower(obj,savef,closefig)
            figure()
            plot(obj.CurveI.*obj.CurveV,obj.CurveV);
            title('Stack Voltage as a Function of Power')
            xlabel('Power (W)')
            ylabel('Voltage (V)')
            if savef
                savefig([obj.OutputFolder Delimiter() 'FuelcellVoltagevsPower.fig'])
            end
            saveas(gcf,[obj.OutputFolder Delimiter() 'FuelcellVoltagevsPower.png'])
            if closefig
                close
            end
        end
